...
export const POST = protegida(async (request) => {
  ...
  if (!body?.municipio_codigo) {
    return Response.json({ error: "municipio_codigo é obrigatório" }, { status: 400 });
  }
  return Response.json(await createCabo(body), { status: 201 });
});

export const PATCH = protegida(async (request) => {
  const body = await request.json();
  if (!body?.id) return Response.json({ error: "id é obrigatório" }, { status: 400 });
  return Response.json(await updateCabo(body));
});

export const DELETE = protegida(async (request) => {
  const body = await request.json();
  if (!body?.id) return Response.json({ error: "id é obrigatório" }, { status: 400 });
  await deleteCabo(body.id);
