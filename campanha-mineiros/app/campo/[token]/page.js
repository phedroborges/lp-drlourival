...
export default function FieldPlanPage({ params }) {
  ...
  const load = useCallback(async () => {
    const response = await fetch(`/api/campo/${encodeURIComponent(token)}`, { cache: "no-store" });
  ...
  async function sharePlan() {
    const data = { title: task.rota_nome, text: `Plano de campo · ${task.municipio_nome}`, url: window.location.href };
  ...
  return (
    <main className="field-page">
      ...
      <div className="field-wrap">
        <section className="field-hero"><div className="field-kicker"><span>Rota programada</span><i className={`field-plan-status ${task.status_calculado}`}>{PLAN_STATUS[task.status_calculado] || "Planejada"}</i></div><h1>{task.rota_nome}</h1><p className="field-area">⌖ {task.municipio_nome}</p><div className="field-schedule"><div><span>Data</span><strong>{formatDate(task.data)}</strong></div><div><span>Turno</span><strong>{task.turno}</strong></div></div>{task.observacao ? <div className="field-brief"><span>Orientação da coordenação</span><p>{task.observacao}</p></div> : null}</section>
        ...
        <section className="field-team-card">
          ...
          <div className="field-team-list">{task.cabos.map((cabo) => <article key={cabo.cabo_id}><span className="field-person-avatar">{cabo.nome.slice(0, 2).toUpperCase()}</span><div><strong>{cabo.nome}</strong><small>Escalado para {task.municipio_nome}</small></div><span className="field-assigned-check">✓</span></article>)}</div>
